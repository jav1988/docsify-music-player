...
歌曲多的时候不用手写每一行。仓库自带 `gen-music-block.py`，扫描一个目录里的音频文件，自动生成上面的 ` ```music ` 代码块，复制粘贴即可。

**不带参数运行会打印帮助：**

```bash
python gen-music-block.py
```
...
```bash
# 扫描 docs/_media/music，路径原样写进每一行
python gen-music-block.py docs/_media/music --title 我的歌单
...
# 文件名是「歌手 - 曲名」顺序，随机播放，直接写到文件
python gen-music-block.py ./music --order artist-title --mode shuffle -o playlist.md
```

**文件名解析**：去掉扩展名后按分隔符（默认 `-`）拆成 `曲名 | 歌手`，例如 `不如见一面-海来阿木&单依纯.mp3` → `不如见一面 | 海来阿木&单依纯`。没有分隔符时整个名字当曲名。

**封面匹配**：默认在音频同目录找同名图片（`.jpg/.png/.webp` 等），匹配不到则不写封面列（前端用占位图）。
...
| --- | --- |
| `--title` | 播放器标题 |
| `--mode` | `sequence` / `listLoop` / `single` / `shuffle`，默认 `listLoop` |
| `--theme` | `auto` / `light` / `dark` |
| `--autoplay` | 加上则写 `autoplay: true` |
| `--sep` | 文件名里 曲名/歌手 的分隔符，默认 `-` |
| `--order` | `title-artist`（默认）/ `artist-title` |
| `--cover-dir` | 封面图所在目录，默认与音频同目录 |
| `--prefix` | 曲目行路径前缀，默认用传入目录；空字符串则只写文件名 |
| `--audio-base` / `--cover-base` | 改为输出 `audioBase` / `coverBase` 配置行，曲目行只写文件名 |
| `-o` / `--output` | 写入文件，默认打印到标准输出 |

纯 Python 3 标准库，无需安装任何依赖。
